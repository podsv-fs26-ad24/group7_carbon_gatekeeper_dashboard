...
The report ensures **traceability and reproducibility** of the results, in particular, how the historical travel records were prepared, and how the CO₂ budgets for 2026 were derived.
...
A heatmap of trip counts shows that **flights dominate every purpose**.
...
Top arrival countries by trip count, ("departure → arrival") and top route *pairs* (collapsed by direction):

![](pics/geographical_analysis.png)

### CO₂ Budget Comparison (Actual vs. Budget)

For the per-BU budget comparison, the subunit-level budgets in `co2_budgets` are aggregated to BU level via the rule `Subunit X.Y → BUX`, then compared with the actual yearly emissions:
