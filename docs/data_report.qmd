...
>
> `traveldataexport.xlsx` (raw export)
>     → manual cleaning in Excel + CO₂-budget assumption for 2026
>     → `traveldataexport_clean.xlsx` (historical reference loaded into the app)
...
| ------------------------- | --------------------------------------------------- | ----------------------------------------------- |
| Travel Data Export (raw)  | Company travel management system (~25,500 records) | `traveldataexport.xlsx`                         |
...
- **Data Governance:** Internal business use. No personally identifiable information is stored — only `person_type` (employee vs. guest).
...
- **Description:** A dynamic list of planned, upcoming trips provided by the travel or sustainability manager and uploaded into the Streamlit dashboard. Used to estimate future emissions against the 2026 budget and to identify modal-shift potential. If no input file is provided, the dashboard falls back to analysing the historical data itself.
- **File Format:** Excel workbook with two sheets: `planned_trips` (the trip list) and `instructions` (German-language template guide).
- **Sample sizes used for testing:**
    - `input_data.xlsx` — **1,936 planned trips** (compact test set; calibrated to push two BUs over budget and contain several flights with a train alternative).
    - `input_data_3.xlsx` — **6,429 planned trips** (larger stress-test set).
...
The raw export `traveldataexport.xlsx` was processed **manually in Microsoft Excel** to produce `traveldataexport_clean.xlsx`. The cleaning step did **not** change any trip records — the cleaned `travel_data` sheet has the same row count (25,527) as the raw export.
...
5. **`co2_budgets` sheet kept as-is** — still 100 rows (10 subunits × 10 years 2017–2026).
...
| `co2_budgets`     | 100 × 3      | Historical subunit-level budgets 2017–2026                             |
...
The exploratory data analysis was carried out in the Jupyter notebook **`eda/eda_travel_data.ipynb`** on the cleaned file `traveldataexport_clean.xlsx`. The goal was to understand structure, distributions and emission drivers of the historical travel data before designing the visualisations.

> Placeholders are marked with **`[PLATZHALTER: …]`** — replace each with the rendered image (or embed the code chunk in Quarto) before publishing.

### Setup & Data Import

The notebook imports `travel_data`, `data_dictionary`, `budget_2026` and `co2_budgets` into pandas DataFrames using `pd.read_excel(...)`. Confirmed shapes:
...
The distributions of distance, CO₂ and cost are **strongly right-skewed** (large gap between the median and the mean / maximum). This is typical for business travel data and motivates the use of log scales in the univariate distribution charts.
...
> **[PLATZHALTER: `fig_4_1_categorical_counts.png` — 2×3 grid of bar plots: trip counts per category for the six categorical variables above.]**
...
> **[PLATZHALTER: `fig_4_2_numeric_boxplots.png` — boxplots (log y-axis) of `km` and `CO2e RFI2 (t)` per `transport_mode`.]**

### Temporal Development

Aggregating on a yearly level reveals the **COVID-19 drop in 2020–2021** and the subsequent recovery. Annual trip counts and CO₂ totals (RFI 2.7):
...
> **[PLATZHALTER: `fig_5_1_yearly_bars.png` — 2×2 bar grid: trips / km / CO₂ / cost per year.]**
>
> **[PLATZHALTER: `fig_5_2_monthly_by_mode.png` — line plot: monthly CO₂ (RFI 2.7) coloured by transport mode, showing the 2020–21 collapse and recovery.]**

### Bivariate / Multivariate Relationships

#### Distance vs. CO₂ by transport mode

A log-log scatter (sample of 5,000 points) shows the expected near-linear relationship between distance and CO₂, with **flights sitting clearly above ground modes** at the same distance.

> **[PLATZHALTER: `fig_6_1_km_vs_co2.png` — log-log scatter `km` vs `CO2e RFI2.7 (t)` coloured by transport mode.]**

#### CO₂ and cost per Business Unit

Aggregated over 2017–2026 (cleaned data):

> **[PLATZHALTER: `fig_6_2_bu_aggregates.png` — table or grouped bars: trips / CO₂ / cost / km per BU.]**

#### Travel purpose × transport mode
...
> **[PLATZHALTER: `fig_6_3_purpose_mode_heatmap.png` — heatmap `travel_purpose` × `transport_mode` with trip counts.]**
...
> **[PLATZHALTER: `fig_6_4_avoidable_co2_per_year.png` — yearly bar chart of avoidable CO₂ from flights with a train alternative.]**
...
> **[PLATZHALTER: `fig_7_1_top_countries.png` — side-by-side bars: top 15 arrival countries by trip count and by CO₂ (RFI 2.7).]**
>
> **[PLATZHALTER: `fig_7_2_top_routes.png` — horizontal bar chart of the 20 most-flown routes.]**
>
> **[PLATZHALTER: `fig_7_3_top_route_pairs.png` — top 20 route *pairs* (alphabetically canonicalised), stacked by outbound / return direction.]**
...
> **[PLATZHALTER: `fig_8_1_actual_vs_budget.png` — small-multiples line chart per BU: actual CO₂ vs. budget over 2020–2025; 2026 covered by the assumption-based `budget_2026` sheet.]**
...
- A pronounced **COVID-19 dip in 2020–2021** is followed by a steady recovery; 2025 trip volume already exceeds 2017 / 2018 levels.
