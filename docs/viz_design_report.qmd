...
| Status banner | budget_utilisation (%) | numeric label + background fill | colour hue (green / yellow / red) | categorical (traffic-light) |
| BU gauges (one per BU) | actual_co2, bu_budget | arc segment + tick mark | angular position (CO₂ value), colour hue (status zone), tick position (budget threshold) | linear (0 → max), categorical zone |
| BU bar chart | business_unit, actual_co2, saving_potential, bu_budget | stacked horizontal bar + dotted reference line | vertical position (business unit), length (CO₂), texture (solid = current / hatched = saving), colour hue (BU identity), horizontal position of marker (budget) | categorical (BU), linear (tonnes) |
...
| Reduction-levers table | origin, destination, alternative_mode, flight_count, avg_flight_co2, avg_alt_co2, saving_t, saving_pct | table row + selectable | vertical position (rank by saving), colour hue (green saving % column) | categorical, linear |
