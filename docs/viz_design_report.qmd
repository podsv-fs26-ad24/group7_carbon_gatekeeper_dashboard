...
## Data-to-Visual Mapping
The dashboard draws on two processed tables produced from the source files: a historical reference table and a planned-trips table. The CO₂ metric used for all calculations is user-selectable (CO₂e RFI2, CO₂e RFI2.7), so the dashboard adapts to whichever standard the organisation reports against. The variables most frequently mapped are origin and destination (IATA codes), mode, distance, business_unit, trip_count, and co2_kg.
...
Two notes on changes I made versus the previous draft, all driven by the code:
