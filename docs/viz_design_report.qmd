...
## Data-to-Visual Mapping
The dashboard draws on two processed tables produced from the source files: a historical reference table (≈25,500 trips, used to derive route × mode averages and BU budgets) and a planned-trips table (the uploaded scenario, here 2,096 trips). The CO₂ metric used for all calculations is user-selectable (e.g. CO₂e, CO₂e RFI2, CO₂e RFI2.7), so the dashboard adapts to whichever standard the organisation reports against. The variables most frequently mapped are origin and destination (IATA codes), mode, distance, business_unit, trip_count, and co2_kg.
...
| Geographic distribution map | origin, destination (coordinates), mode, route_co2 | great-circle line + point | spatial position (lat/lon), line width (CO₂), colour hue (mode), point (airport node) | geographic (Equal Earth), linear (width), categorical (mode) |
| Reduction-levers table | origin, destination, alternative_mode, flight_count, avg_flight_co2, avg_alt_co2, saving_t, saving_pct | table row + checkbox | position (row order = saving rank), colour hue (saving % in green) | categorical, linear |
...
CO₂ is consistently shown on linear scales across the dashboard. The BU bar chart uses a shared linear x-axis in tonnes so that bars are directly comparable across units. The route deep-dive uses a dual linear axis, with CO₂ per passenger on the left y-axis and duration on the right, because the two metrics have different units but the user needs to read them in parallel. The geographic map uses an Equal Earth projection, chosen over Mercator because it preserves relative area and avoids exaggerating the visual weight of high-latitude intercontinental routes. Categorical scales are used for transport mode, Business Unit, and origin/destination. Axes are labelled in plain units (CO2 (t), CO2 (kg/pax), Duration (h)) and gauge ticks are placed at intervals that align with the underlying budget for that BU.

### Colour design
Two semantic palettes run through the dashboard.
A traffic-light palette (green / yellow / red) encodes budget status and is used in the gauge zones, the status banners, and the green saving-percentage column in the reduction-levers table. The colours are mapped to fixed thresholds (under, approaching, over budget), so colour carries information independent of legend lookup.
